...
class GazeDetector(BaseDetector):
    ...
    def landmark_vals(self, frame):
        ...
        lm_result = self.face_landmarker.detect(mp_frame)
    ...
    def full_landmark_mapping(self, lm_result, frame): # does the full face, just testing
    ...
    def eyes_landmark_mapping(self, lm_result, frame): # found the eye idx vals
    ...
    def eye_blink_threshold(self, lm_le_range, lm_re_range, one_sec_n_frames=25, threshold_frames=3, n_threshold=0.15, b_duration=1):
        '''
    ...
    def eye_staring_tracker(self, blink_threshold_func, one_sec_n_frames, staring_duration=10):
        ...
        if blink_threshold_func: # if true
            ...
            if self.staring_seconds > n_frames: # has threshold been reached
                return True # staring
            else:
                return False # not staring
    ...
    def gaze_direct_detect(self, lm_result, frame, c_threshold=0.8):
    ...
    def gaze_duration_detect(self, lm_result, frame, one_sec_n_frames, gaze_threshold=20, c_threshold=0.8):
        ...
        if direction == 3:
            ...
            self.gaze_down_frames = 0 # resets back to zero if user stops looking down
            return False
    ...
    #====================
    #   final integration -- yeahhhh didnt work outside module
    #==========================
    ...
    def run(self):
        ...
        while True:
            ...
            one_sec_n_frames = self.seconds_to_frames(self.num_frames_count, self.start_time, 1)
            ...
            if lm_result and len(lm_result.face_landmarks) >= 1:
                ...
                if self.eye_sleeping_threshold(self.lm_le_range, self.lm_re_range, one_sec_n_frames):
                    self.alert.process(DetectionResult(
                        module_name="gaze", is_ok=False,
                        warning_message="You look drowsy",
                        confidence=0.9
                    ))

                eye_b_thresh = self.eye_blink_threshold(self.lm_le_range, self.lm_re_range, one_sec_n_frames, threshold_frames=1)
                if self.eye_staring_tracker(eye_b_thresh, one_sec_n_frames):
                    self.alert.process(DetectionResult(
                        module_name="gaze", is_ok=False,
                        warning_message="Remember to blink!",
                        confidence=0.9
                    ))

                if self.gaze_duration_detect(lm_result, frame, one_sec_n_frames):
                    self.alert.process(DetectionResult(
                        module_name="gaze", is_ok=False,
    ...
    def analyze(self, frame: np.ndarray) -> DetectionResult:
        ...
        if not lm_result or len(lm_result.face_landmarks) == 0: # not exactly needed but might stop it crashing
        ...
        if self.eye_sleeping_threshold(self.lm_le_range, self.lm_re_range, one_sec_n_frames): # checks if theyre sleeping
            return DetectionResult(
                module_name="gaze",
                is_ok=False,
        ...
        eye_b_thresh = self.eye_blink_threshold(self.lm_le_range, self.lm_re_range, one_sec_n_frames,threshold_frames=1)
        if self.eye_staring_tracker(eye_b_thresh, one_sec_n_frames): # checks if theyre staring at the screen
            return DetectionResult(
                module_name="gaze",
                is_ok=False,
                warning_message="Remember to blink!",
        ...
        if self.gaze_duration_detect(lm_result, frame, one_sec_n_frames): # checks if they arent looking at the screen
            return DetectionResult(
                module_name="gaze",
                is_ok=False,
                warning_message="You've been looking away from the screen too long!",
...
#==========================================================
#==========================================================
#   grab the web cam feed - stand alone - not integrated
#==========================================================
# #==========================================================
#
