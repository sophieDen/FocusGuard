...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Process both pose and objects synchronously using the same timestamp
        ...
        try:
            ...
            elif shoulder_lvl_difference > 20: # 2-2. Check if user's shoulder is leaning one side
                good_frames = 0
                bad_frames += 1
                color = red
                suggestions = "Your shoulders are uneven."
            elif neck_inclination > 35: # 2-3-1. Check if user's neck is tilted (right)
                good_frames = 0
                bad_frames += 1
                color = red
                suggestions = "Straighten your neck."
            elif neck_inclination < 15: # 2-3-2. Check if user's neck is tilted to the other side (left)
                good_frames = 0
                bad_frames += 1
                color = red
                suggestions = "Straighten your neck."
                
            # 3. If none of the issue is detected, proceed with outputting good posture result
            else:
                good_frames = 1
                bad_frames += 0
                color = light_green
